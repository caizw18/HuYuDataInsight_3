# This is the code for ResNet model based on blocks of version 1 (Standard Block)
...
# Define the class of Standard_Block
class StandardBlock(nn.Module):
    # Expansion is how many times the output dimension in the residual structure is the input dimension.
    # StandardBlock doesn't increase the dimension, so expansion = 1
    expansion = 1

    def __init__(self, inputs, outputs, stride=1):
        super(StandardBlock, self).__init__()
        # The first 3x3 convolution for Standard Block
        ...
        # The second 3x3 convolution for Standard Block
        ...
        # the addition successfully in standard_block, we shortcut the identity via passed convolution layer
    ...
    def forward(self, x):
        # First 3x3 convolution, then batch normalization, then ReLU
        out_x = F.relu(self.bn1(self.conv1(x)))
        # Second 3x3 convolution, then batch normalization
        out_x = self.bn2(self.conv2(out_x))
...
# Define the ResNet based on Standard Block
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10): # 10 classes of images classification
        ...
        # Layers built by Standard Block, the number of blocks in every layer depends on the input list
        # For example, if the input list is [2,2,2,2], then every layer has 2 standard blocks
        # The total number of layers is 1(conv1) + 2x2x4 + 1(fc layer) = 18, so it is ResNet-18
...
# Our model here is based on ResNet (standard block version) with layers [2,2,2,2]
model = ResNet(StandardBlock, [2, 2, 2, 2]).to(Dev)
...
with torch.no_grad():
    ...
    str = "The accuracy of ResNet model based on Standard Block is {} %".format(correct_pre / num_total * 100)
    print(str)
